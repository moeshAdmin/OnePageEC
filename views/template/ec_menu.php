...
$logoURL = SITE_IMAGE;
if(!$topMenu){
  $topMenuStyle = 'style="display:none;"';
}
?>
<!-- 選單資訊 (ex. MenuBar, Navigation Bar, ...) -->
<div id="topMenu" class="container animated fadeIn" <?php echo $topMenuStyle; ?>>
      ...
      <ul class="navbar-nav">
        <?php 
          foreach ($topMenu as $key => $value) {
            if($value['name']=='會員專區'&&!$user_detail['m_email']){$value['name'] = '會員登入';}
            echo 
